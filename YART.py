#!/usr/bin/env python
#run with 'python YART.py -pl plink.map -lm .LMmap file > nameOfOutputFile
...
def doWork(lineNum, d, temp, lm, corrected):
    l = d[lineNum]
    a = l[0]
    #format the output
    temp=[cleanId(a[0]), lm[3]]
    #append to list
	    ...
	    #print i
            ...
            if len(lm) > 0:
                ...
                if not str(lm[0]).startswith('#'):
                    doWork(lm[0], d, temp, lm, corrected)
            lm [:] = []
    #print
